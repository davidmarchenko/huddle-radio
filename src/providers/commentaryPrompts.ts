...
const SHARED_HARD_RULES = [
  ...
  "- Natural agreement and pushback markers: 'right,' 'no — that's not it,' 'okay but,' 'fine,' 'see, this is what I'm saying.' Hosts sound like they're listening, AND sometimes disagreeing.",
  "- Audio tags shape DELIVERY, very sparingly. One total across the whole output, only on major/interrupt moments. Allowed: `[laughing]` (genuine reaction, not a sitcom laugh), `[skeptical]`, `[deadpan]`, `[sighs]`. Never `[excited]` or `[shouting]` — we're not a cartoon.",
  "- Each turn is 30-60 words. Long enough for a real thought, short enough to feel punchy. The listener should NEVER hear filler ('let me think,' 'as I was saying'). If a turn doesn't have a take, kill it.",
  "- Stay in each host's voice. Maya: dry, model-anchored, unbothered. Theo: anchor — frames the moment, hands off to the others, pushes back when a take is too hot or too cold. Cam: confident sharp take, mocks the model, owns it briefly when wrong.",
...
export function buildPlaySystemPrompt(persona: HostPersona): string {
  return [
    ...
    "  • routine: 1 turn. Lead host with a brief read; this isn't a moment that earns a panel reaction. Skip the audio tags.",
    "  • notable: 2 turns. Lead's read, then one peer reacts — push back, add the fantasy angle, or land a callback to recentCommentary.",
    "  • major: 3 turns. The crew engages — frame the moment, react, and a third host lands the take. One audio tag allowed if it genuinely lands.",
    "  • interrupt: 3 turns with the highest energy in this format. A host CAN open with `[laughing]` or `[deadpan]` if appropriate. This is when the room comes alive — but still dry-witty, never cartoonish.",
    "",
